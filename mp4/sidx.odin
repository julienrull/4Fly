...
deserialize_sidx :: proc(data: []byte) ->  (sidx: Sidx, acc: u64) {
    ...
    for i:=0; i < int(sidx.reference_count); i+=1 {
        ...
        sidx.items[i].reference_type = (byte)((tmp & 0x80000000)>>24)
        sidx.items[i].referenced_size = tmp & 0x7fffffff
        acc += size_of(u32be)
        sidx.items[i].subsegment_duration = (^u32be)(&data[acc])^
        acc += size_of(u32be)
        tmp = (^u32be)(&data[acc])^
        sidx.items[i].starts_with_SAP = (byte)((tmp & 0x80000000)>>24)
        sidx.items[i].SAP_type = (byte)((tmp & 0x70000000)>>24)
        sidx.items[i].SAP_delta_time = tmp & 0xfffffff
        acc += size_of(u32be)
    }
    return sidx, acc
}
...
serialize_sidx :: proc(sidx: Sidx) ->  (data: []byte) {
    ...
    for i:=0; i < int(sidx.reference_count); i+=1 {
        ...
        tmp := [4]byte(reference_type | (referenced_size << 1))
        acc += size_of(u32be)
